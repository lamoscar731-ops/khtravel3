...
const App: React.FC = () => {
  ...
  const [trips, setTrips] = useState<Trip[]>(() => {
      ...
      // Migration for single-trip data
      ...
      // Default template if completely empty
      ...
      // SANITIZE: Force every field to be correct type
  ...
  // Reload state when active trip changes
  useEffect(() => {
  ...
  // Sync state back to trips array
  useEffect(() => {
      setTrips(prevTrips => {
  ...
  const handleDeleteTrip = () => { vibrate(); if (trips.length <= 1) { alert("Keep one trip."); return; } if (confirm("Delete?")) { const newTrips = trips.filter(t => t.id !== activeTripId); setTrips(newTrips); localStorage.setItem('kuro_trips', JSON.stringify(newTrips)); const next = newTrips[0]; setActiveTripId(next.id); setShowSettings(false); } };
