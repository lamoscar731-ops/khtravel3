...
const App: React.FC = () => {
  ...
  useEffect(() => { localStorage.setItem('kuro_lang', lang); }, [lang]);
  useEffect(() => { const timer = setInterval(() => setNow(new Date()), 60000); return () => clearInterval(timer); }, []);
  
  useEffect(() => {
      fetch('https://api.exchangerate-api.com/v4/latest/HKD').then(res => res.json()).then(data => {
            if (data && data.rates) {
                const newRates: Record<string, number> = {};
                Object.keys(DEFAULT_RATES).forEach(key => { if (data.rates[key]) newRates[key] = 1 / data.rates[key]; else newRates[key] = DEFAULT_RATES[key]; });
  ...
  const [activeTripId, setActiveTripId] = useState<string>(() => { return localStorage.getItem('kuro_active_trip_id') || (trips.length > 0 ? trips[0].id : ''); });
  ...
  const isLiveItem = (item: ItineraryItem, index: number, items: ItineraryItem[]) => {
      ...
      if (index < items.length - 1) nextItemMinutes = parseInt(items[index + 1].time.split(':')[0]) * 60 + parseInt(items[index + 1].time.split(':')[1]);
      else nextItemMinutes = itemMinutes + 120; 
      return currentMinutes >= itemMinutes && currentMinutes < nextItemMinutes;
  };

  const handleCreateTrip = () => { vibrate(); const newTrip: Trip = { id: `trip-${Date.now()}`, destination: 'NEW TRIP', startDate: new Date().toISOString().split('T')[0], itinerary: [{ dayId: 1, date: new Date().toISOString().split('T')[0], items: [] }], flights: [], hotels: [], budget: [], contacts: [], totalBudget: 20000, checklist: [], notes: '', coverImage: '' }; setTrips(prev => [...prev, newTrip]); setActiveTripId(newTrip.id); setActiveTab(Tab.ITINERARY); };
  const handleDeleteTrip = () => { vibrate(); if (trips.length <= 1) { alert("Keep one trip."); return; } if (confirm("Delete?")) { const newTrips = trips.filter(t => t.id !== activeTripId); setTrips(newTrips); localStorage.setItem('kuro_trips', JSON.stringify(newTrips)); setActiveTripId(newTrips[0].id); setShowSettings(false); } };
  ...
  const handleToggleChecklist = (id: string) => setChecklist(p => p.map(i => i.id === id ? { ...i, checked: !i.checked } : i));
