...
const HotelItem: React.FC<{ hotel: HotelInfo, onUpdate: (h: HotelInfo) => void, onDelete: (id: string) => void, lang: Language }> = ({ hotel, onUpdate, onDelete, lang }) => {
    ...
    return (
        <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-3 shadow-sm mb-3 relative group">
            ...
            <div className="flex justify-between items-start mb-1 pr-16"><h3 className="text-sm text-white font-medium">{formData.name}</h3></div>
            <p className="text-[10px] text-neutral-400 mb-3 leading-relaxed">{formData.address}</p>
            <div className="grid grid-cols-2 gap-4 border-t border-neutral-800 pt-2"><div><div className="text-[9px] text-neutral-600 uppercase">Check In</div><div className="text-xs text-white">{formData.checkIn}</div></div><div><div className="text-[9px] text-neutral-600 uppercase">Booking Ref</div><div className="text-xs text-amber-200 font-mono">{formData.bookingRef}</div></div></div>
        </div>
    );
};
        ...
        return (
            <div className="bg-neutral-800 border border-neutral-700 p-2 rounded-lg flex flex-col gap-1 relative">
                ...
                <div className="flex justify-end gap-2 mt-1">
                     <button onClick={() => { vibrate(); onDelete(item.id); }} className="text-red-400 text-[10px] px-1">{T.DELETE[lang]}</button>
                     <button onClick={handleSave} className="bg-white text-black text-[10px] px-2 py-0.5 rounded font-bold">{T.DONE[lang]}</button>
                </div>
            </div>
        )
    }
        ...
        return (
             <div className="p-3 border-b border-neutral-800 bg-neutral-800/50">
                ...
                <div className="flex justify-between items-center">
                    <select value={data.category} onChange={(e) => setData({...data, category: e.target.value})} className="bg-transparent border-b border-neutral-600 text-neutral-400 text-[10px] focus:outline-none w-1/2 appearance-none uppercase">{Object.values(ItemType).map(t => <option key={t} value={t} className="bg-neutral-900 text-white">{t}</option>)}</select>
                    <div className="flex gap-2"><button onClick={() => { vibrate(); onDelete(item.id); }} className="text-red-400 text-[10px] px-2 border border-red-900/50 rounded">{T.DELETE[lang]}</button><button onClick={handleSave} className="bg-white text-black text-[10px] px-2 py-0.5 rounded font-bold">OK</button></div>
                </div>
             </div>
        )
    }
    ...
    <div className="space-y-4 pb-24">
      ...
      <section>
        ...
        <div className="bg-neutral-900 border border-neutral-800 rounded-lg overflow-hidden">
            {budget.map((item) => <BudgetItem key={item.id} item={item} onUpdate={onUpdateBudget} onDelete={onDeleteBudget} rates={rates} lang={lang} />)}
            <button onClick={onAddBudget} className="w-full py-2 text-[10px] text-neutral-500 hover:text-white hover:bg-neutral-800 transition border-b border-neutral-800">+ {T.ADD_EXPENSE[lang]}</button>
        </div>
      </section>

      <section>
          <div className="flex justify-between items-end mb-2 ml-1">
              <h2 className="text-neutral-500 text-[10px] font-bold tracking-widest uppercase">{T.PACKING_LIST[lang]}</h2>
              <button onClick={onAiChecklist} disabled={isLoadingAi} className="text-amber-200 hover:text-amber-100 text-[10px] flex items-center gap-1 uppercase">
                  {isLoadingAi ? <span className="animate-pulse">Thinking...</span> : <><span>✨ {T.AI_SUGGEST[lang]}</span></>}
              </button>
          </div>
          <div className="bg-neutral-900 border border-neutral-800 rounded-lg overflow-hidden">
              ...
              <div>
                  {checklist?.map(item => (
                      <div key={item.id} className="flex items-center gap-3 p-3 border-b border-neutral-800 last:border-0 hover:bg-neutral-800/30 group">
                          <button onClick={() => onToggleChecklist(item.id)} className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors ${item.checked ? 'bg-white border-white' : 'border-neutral-600 hover:border-neutral-400'}`}>
