...
const TypeIcon: React.FC<{ type: ItemType }> = ({ type }) => {
  switch (type) {
    case ItemType.FOOD: return <span className="text-sm">🥢</span>;
    case ItemType.TRANSPORT: return <span className="text-sm">🚄</span>;
    case ItemType.SHOPPING: return <span className="text-sm">🛍️</span>;
    case ItemType.HOTEL: return <span className="text-sm">🏨</span>;
        ...
        <div className="flex gap-3 mb-2 relative">
            ...
            <div className="flex flex-col items-center min-w-[32px] z-10">
                ...
                <div className="w-8 h-8 rounded-full bg-neutral-900 border border-neutral-700 flex items-center justify-center relative shadow-inner">
                    ...
                    <div className="absolute -bottom-1 -right-1 w-3 h-3 bg-neutral-800 rounded-full border border-neutral-600 flex items-center justify-center">
                ...
                <div className="space-y-3">
                    <div className="grid grid-cols-3 gap-2">
                        ...
                        <div className="col-span-2">
                             ...
                             <select value={formData.type} onChange={(e) => handleChange('type', e.target.value)} className="w-full bg-transparent border-b border-neutral-700 text-white text-sm py-0.5 focus:outline-none focus:border-neutral-400 appearance-none bg-neutral-900">
                    ...
                    <div>
                        ...
                        <input type="text" value={formData.title} onChange={(e) => handleChange('title', e.target.value)} className="w-full bg-transparent border-b border-neutral-700 text-white font-bold text-base py-0.5 focus:outline-none focus:border-neutral-400" />
                    </div>
                    <div>
                        <label className="text-[9px] text-neutral-500 font-bold block mb-0.5">Location</label>
                        <input type="text" value={formData.location} onChange={(e) => handleChange('location', e.target.value)} className="w-full bg-transparent border-b border-neutral-700 text-neutral-300 text-xs py-0.5 focus:outline-none focus:border-neutral-400" />
                    </div>
                    <div>
                        ...
                        <div className="flex gap-1 items-center bg-neutral-800/50 p-1.5 rounded border border-neutral-700/50">
                            <input type="text" value={newTagLabel} onChange={(e) => setNewTagLabel(e.target.value)} placeholder="New tag" className="flex-1 bg-transparent text-white text-[10px] placeholder-neutral-600 focus:outline-none" onKeyDown={(e) => e.key === 'Enter' && handleAddTag()} />
                    ...
                    <div>
                        ...
                        <textarea value={formData.description || ''} onChange={(e) => handleChange('description', e.target.value)} rows={3} className="w-full bg-transparent border-b border-neutral-700 text-neutral-400 text-xs py-0.5 focus:outline-none focus:border-neutral-400 resize-none leading-relaxed normal-case" placeholder="Description..." />
                    </div>
                    <div className="flex gap-2 pt-1">
                        <button onClick={handleSave} className="flex-1 bg-neutral-100 text-black py-1.5 rounded text-xs font-bold hover:bg-white">Save</button>
                        <button onClick={() => setIsEditing(false)} className="flex-1 bg-neutral-800 text-neutral-300 py-1.5 rounded text-xs font-bold hover:bg-neutral-700">Cancel</button>
                        <button onClick={() => onDelete(item.id)} className="w-8 bg-red-950/30 text-red-400 border border-red-900/50 rounded flex items-center justify-center hover:bg-red-900/50">🗑️</button>
                    </div>
                </div>
            </div>
        </div>
      ...
      <div className="flex-1 bg-neutral-900 border border-neutral-800 rounded-lg p-3 shadow-sm mb-3 relative transition-colors hover:border-neutral-700">
        ...
        <div className="flex justify-between items-start mb-1 pr-6">
            <div>
                ...
                {item.weather && <div className="text-xs text-blue-300 mt-0.5 flex items-center gap-1">☁️ {item.weather}</div>}
            </div>
            <div className="flex flex-col items-end gap-1">
                {item.tags?.map((tag, idx) => (
                    <span key={idx} className={`text-[9px] px-1.5 py-0.5 rounded border tracking-wider ${tag.color === 'gold' ? 'text-amber-200 border-amber-900 bg-amber-950/30' : tag.color === 'red' ? 'text-red-300 border-red-900 bg-red-950/30' : 'text-neutral-400 border-neutral-700 bg-neutral-800'}`}>{tag.label}</span>
                ))}
            </div>
        </div>
        <p className="text-xs text-neutral-400 leading-relaxed mb-2 whitespace-pre-wrap">{item.description}</p>
        {item.tips && item.tips.length > 0 && (
            <div className="mb-3 bg-neutral-950/50 p-2 rounded border border-neutral-800/50">
                <p className="text-[9px] text-neutral-500 uppercase tracking-widest mb-1 font-bold">Guide Notes</p>
                <ul className="list-none space-y-1">
                    {item.tips.map((tip, idx) => (
                        <li key={idx} className="text-[10px] text-neutral-300 flex items-start gap-1.5">
        ...
        <div className="flex items-center justify-between mt-2 pt-2 border-t border-neutral-800">
            <span className="text-[10px] text-neutral-600 truncate max-w-[150px]">{item.location}</span>
            <button onClick={handleNavClick} className="flex items-center gap-1.5 bg-neutral-100 text-black px-3 py-1 rounded-full text-[10px] font-bold hover:bg-neutral-300 transition-colors">
                <span>Navigate</span>
